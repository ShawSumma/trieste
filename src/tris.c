...
tri_t tri_join_from_ptr(tri_table_t *table, const tri_t *next) {
...

    {
        float value = 0;
        for (int i = 0; i < 4; i++) {
            float part = next[i].color.r * 0.00390625f;
            value += part * part;
        }
        write->color.r = sqrtf(value) * 128.0f;
    }
    {
        float value = 0;
        for (int i = 0; i < 4; i++) {
            float part = next[i].color.g * 0.00390625f;
            value += part * part;
        }
        write->color.g = sqrtf(value) * 128.0f;
    }
    {
        float value = 0;
        for (int i = 0; i < 4; i++) {
            float part = next[i].color.b * 0.00390625f;
            value += part * part;
        }
        write->color.b = sqrtf(value) * 128.0f;
    }
